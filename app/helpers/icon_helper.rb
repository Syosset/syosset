# Modified version of font-awesome-rails icon helper (adds "aria-hidden" => true automatically)
module IconHelper
  def fa_icon(names = 'flag', original_options = {})
    ...
    icon = content_tag(:i, nil, options.merge(class: classes, "aria-hidden" => true))
  ...
  def fa_stacked_icon(names = 'flag', original_options = {})
    ...
    text = options.delete(:text)
    right_icon = options.delete(:right)
    stacked_icon = content_tag(:span, safe_join(icons), options.merge(class: classes, "aria-hidden" => true))
